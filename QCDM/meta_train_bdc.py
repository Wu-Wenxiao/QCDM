...
# from methods.meta_deepbdcv2_sigmoid import MetaDeepBDC
...
def train(params, base_loader, val_loader, model, stop_epoch):
    ...
    #             {'params':model.dcov.parameters(), 'lr':1e-4,},
    #             {'params':model.regularizer.parameters(), 'lr':params.lr,}
    #             ])
    ...
    for epoch in range(0, stop_epoch):
        ...
        #     # optimizer = torch.optim.Adam([
        #     #     {'params':model.regularizer_ins.parameters(), 'lr':1e-3,},
        #     #     {'params':model.regularizer_channel.parameters(), 'lr':1e-3,}
        #     #     ])
        #     # optimizer = torch.optim.SGD(model.regularizer_ins.parameters(), lr=params.lr, momentum=0.9, nesterov=True, weight_decay=5e-4)
        #     optimizer = torch.optim.Adam(model.regularizer_ins.parameters(), lr=params.lr)
        #     # lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=params.milestones, gamma=params.gamma)
        # else:
        #     optimizer = torch.optim.SGD(model.parameters(), lr=params.lr, momentum=0.9, nesterov=True, weight_decay=5e-4)
        # lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=params.milestones, gamma=params.gamma)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--extra_dir', default='-BDC-adapt-v3-weight-next', help='record additional information')
