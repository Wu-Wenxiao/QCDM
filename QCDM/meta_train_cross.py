...
def train(params, base_loader, val_loader, model, stop_epoch):
    ...
    print("adapt all parameters")
    optimizer = torch.optim.SGD(model.parameters(), lr=params.lr, momentum=0.9, nesterov=True, weight_decay=5e-4)
    # k=0.1
    # print(k)
    # lr_fea = params.lr*k
    # optimizer = torch.optim.SGD(
    #                             [{'params':model.feature.parameters(), 'lr':params.lr, 'momentum':0.9, 'nesterov':True, 'weight_decay':5e-4},
    #                             {'params':model.dcov.parameters(), 'lr':params.lr, 'momentum':0.9, 'nesterov':True, 'weight_decay':5e-4},
    #                             {'params':model.regularizer.parameters(), 'lr':lr_fea, 'momentum':0.9, 'nesterov':True, 'weight_decay':5e-4}])
    # print("adapt only regularizer parameters")
    ...
    for epoch in range(0, stop_epoch):
        ...
        #     # optimizer = torch.optim.SGD(model.parameters(), lr=params.lr, momentum=0.9, nesterov=True, weight_decay=5e-4)
        # lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=params.milestones, gamma=params.gamma)
        # print('lr_feature {:f} | lr_regularizer {:f} | optimizer {:s}'.format(optimizer.param_groups[0]['lr'], optimizer.param_groups[-1]['lr'], str(optimizer.__class__)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--extra_dir', default='-mini_to_cars_v3_layernormlast_all-weight', help='record additional information')
    ...
    # val_data_path="/home/dancer/filelists/cub/"
    # val_data_path="/home/dancer/filelists/car_all/cars_for_fsl_split/"
    # val_data_path="/home/dancer/filelists/car_all/"
    val_data_path="/home/dancer/filelists/cars/"
    # val_data_path="/home/dancer/filelists/Aircraft_fewshot/"
    if "Aircraft_fewshot" in val_data_path:
        print("Aircraft_fewshot")
        val_file = 'val_pre'
    print("val_data_path: {:s}".format(val_data_path))
    test_few_shot_params = dict(n_way=params.val_n_way, n_support=params.n_shot)
    val_datamgr = SetDataManager(val_data_path, params.image_size, n_query=params.n_query, n_episode=params.val_n_episode, json_read=True, **test_few_shot_params)
