...
class ProtoNet(MetaTemplate):
    def __init__(self, params, model_func, n_way, n_support):
        ...
        self.k = 0.0
        self.m = 0.0
        print("v3max-crop: layer_norm+feature_dim-adapt+init_weight_crop")
    ...
    def set_forward_adapt(self,x,is_feature = False):
        ...
        for i in range(self.n_way*self.n_query):
            if i == 0 :
                output_raw = self.regularizer(input[i].T).T
                mask = torch.zeros_like(output_raw)
                value, indices=output_raw.topk(256,1)
                mask[:,indices]=1
                output= mask*output_raw
            else:
                output_raw = self.regularizer(input[i].T).T
                ...
                value, indices=output_raw.topk(256,1)
                mask[:,indices]=1
                output_raw = mask*output_raw
                output = torch.cat((output, output_raw),dim = 0)
        ...
        # mask = torch.zeros_like(output_f)
        # # print(z_query.shape[-1])
        # value, indices=output_f.topk(256,2)
        # mask[indices]=1
        # ipdb.set_trace()
