...
class ProtoNet(MetaTemplate):
    def __init__(self, params, model_func, n_way, n_support):
        ...
        self.k = 0.0
        self.m = 0.0
        print("v3_conv: layer_norm+feature_dim-adapt+conv")
        ...
        self.regularizer = nn.Sequential(
                ...
                nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False),
                nn.ReLU(inplace=True),
                nn.AdaptiveAvgPool2d((1, params.reduce_dim)))
    ...
    def set_forward_adapt(self,x,is_feature = False):
        ...
        #     if i == 0 :
        #         output = self.regularizer(input[i])
        #     else:
        #         output = torch.cat((output, self.regularizer(input[i])),dim = 0)
        output = output.contiguous().view(self.n_way*self.n_query, -1)
        # print(output.shape[-1])
        # print(z_query.shape[-1])
        assert output.shape[-1] == z_query.shape[-1]
