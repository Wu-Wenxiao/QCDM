...
class MetaTemplate(nn.Module):
    ...
    def parse_feature_each(self, x, is_feature):
        # x = Variable(x.cuda()).half()
        x = Variable(x.cuda())
        if is_feature:
            z_all = x
            ...
            z_all = self.feature_forward(x)
            # z_all = z_all.view(self.n_way, self.n_support + self.n_query, -1)
        z_support = z_all[:-1].view(self.n_way, self.n_support, -1)
        z_query = z_all[-1]
    ...
    def train_loop_sup(self, epoch, train_loader, optimizer):
        print_freq = 200
        avg_loss = 0
        avg_cla = 0
        avg_part = 0
        ...
        for i, (x, _) in enumerate(train_loader):
            ...
            if self.change_way:
                self.n_way = x.size(0)
            optimizer.zero_grad()
            correct_this, count_this, loss_cla, loss_part, _ = self.set_forward_loss_sup(x)
            # loss = (1-self.nambda)*loss_cla + self.nambda*loss_part
            loss = loss_cla + self.nambda*loss_part
            acc_all.append(correct_this / count_this * 100)
            loss.backward()
            optimizer.step()
            avg_loss = avg_loss + loss.item()
            avg_cla = avg_cla + loss_cla.item()
            avg_part = avg_part + loss_part.item()

            if i % print_freq == 0:
                print('Epoch {:d} | Batch {:d}/{:d} | Loss {:f} | Loss-cla {:f} | Loss-sup {:f}'.format(epoch, i, len(train_loader),
                                                                        avg_loss / float(i + 1), avg_cla / float(i + 1), avg_part/ float(i + 1)))
        acc_all = np.asarray(acc_all)
        acc_mean = np.mean(acc_all)
        return avg_loss / iter_num, acc_mean

    def test_loop_sup(self, test_loader, record=None):
        acc_all = []
        avg_loss = 0
        iter_num = len(test_loader)
        with torch.no_grad():
