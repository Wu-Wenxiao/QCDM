...
# from methods.meta_deepbdcv2max import MetaDeepBDC
...
parser.add_argument('--image_size', default=84, type=int, choices=[32, 84, 224])
parser.add_argument('--dataset', default='mini_imagenet', choices=['mini_imagenet', 'tiered_imagenet', 'cub', 'cars', 'aircraft', 'cifar'])
...
if params.dataset in ['cub', 'cars']:
    novel_file = 'novel.json'
    json_file_read = True
elif params.dataset in ['aircraft']:
    novel_file = 'test'
    json_file_read = False
else:
    novel_file = 'test'
...
for _ in range(params.test_task_nums):
    ...
    for _, (x, _) in enumerate(tqdm_gen):
        with torch.no_grad():
            ...
            flag = torch.ones((80,5)).cuda()
            _, indice = flag.topk(5, dim=1, sorted=True)
            x = x.contiguous().view(model.n_way * (model.n_support + model.n_query), *x.size()[2:])
            x = x.view(model.n_way, (model.n_support + model.n_query), *x.size()[1:])
            x_s = x[:, :model.n_support, :, :, :]
            # x_s = x[:, :model.n_support, :, :, :].contiguous().view(model.n_way *  model.n_support, *x.size()[2:])
            x_q = x[:, model.n_support:, :, :, :].contiguous().view(model.n_way *  model.n_query, *x.size()[2:])
            scores = model.set_forward_each(x_s, x_q, torch.sort(indice)[0], False)

            _, temp_indice = (-scores).topk(1, dim=1, sorted=True)
            flag = flag.scatter(1, temp_indice, 0)
            _, indice = flag.topk(4, dim=1, sorted=True)
            scores = model.set_forward_each(x_s, x_q, torch.sort(indice)[0], False)
            _, last = scores.topk(1, dim=1, sorted=True)

            pred = torch.gather(torch.sort(indice)[0],1, last)
            pred=pred.squeeze(1)
    ...
    acc_all_mean.append(str(acc_mean)[:5])

acc_all_task_mean = np.mean(acc_all_task)
print('%d test mean acc = %4.2f%%' % (params.test_task_nums, acc_all_task_mean))

with open('./record/test_result.txt', 'a') as f:         
        # epoch = params.save_iter
        f.write('outputpath: %s, record: %s, dataset: %s, model: %s, %d times Acc: %4.2f%% \n' %(params.model_path, acc_all_mean[:], params.dataset, params.model, params.test_n_episode, acc_all_task_mean))
