...
for _ in range(params.test_task_nums):
    ...
    for _, (x, _) in enumerate(tqdm_gen):
        with torch.no_grad():
            ...
            # x = x.view(model.n_way, (model.n_support + model.n_query), *x.size()[2:])
            ...
            # scores, _ = model.set_forward_adapt(x, False)
            scores = model.set_forward_adapt(x, False) # v4
            # scores, _ = model.set_forward_sup(x, False)
